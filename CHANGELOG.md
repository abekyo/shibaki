...
- Package renamed from `shibaki` to `shibaki-cli` on npm. (npm rejected the original `shibaki` name as too similar to the existing `shiki` package under their typosquatting heuristic.) v0.2.0 was tagged but never published; v0.2.1 is the first version actually live on npm. The CLI binary remains `shibaki` — only the install identifier changed (`bunx shibaki-cli@latest demo`, `bun add -g shibaki-cli`).
