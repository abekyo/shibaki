...
```bash
# Claude Code (実作業エージェント)
npm install -g @anthropic-ai/claude-code
claude login

# critic 用の API key — agent とは別の provider が必要。
# Gemini は無料・カード不要: https://aistudio.google.com/apikey
...
なぜ critic 用に別 key が要るか: Shibaki は default で main agent とは
**別 provider** の LLM を critic に使う。「自分の出力を自分で擁護する」
self-critique blind spot を避けるため。override 方法は [SECURITY.md](./SECURITY.md)。
...
main provider と critic provider が同一の場合は起動時に拒否される
